...
function FilterBar({
  ...
  return (
    <form className="esmera-report-filters" onSubmit={(event) => { event.preventDefault(); onApply() }}>
      <div className="esmera-report-filter-grid">
      ...
      <div className="esmera-report-filter-actions">
        <button className="esmera-button esmera-button--primary" type="submit">Aplicar filtros</button>
...
function WorkspaceInner({ initialData, users, products, categories }: Props) {
  ...
  function applyFilters() {
    navigate(filtersFromDraft(draft))
  }
  ...
  function patchFilters(patch: Partial<NormalizedReportingFilters>) {
    navigate({ ...activeFilters, ...patch })
  }

  function openDrilldown(kind: ReportingDrilldownKind, value: string | number | null = null) {
    setDrilldownRequest({ kind, value: value === null ? null : String(value) })
  }
  ...
  return (
    <div className={`esmera-report-workspace${reportQuery.isFetching ? ' is-refreshing' : ''}`} data-testid="reports-workspace">
      ...
      <section className="esmera-report-kpis" aria-label="Indicadores comerciais">
        <button type="button" onClick={() => openDrilldown('opportunities')}><span>Oportunidades</span><strong>{metrics.opportunitiesCreated.toLocaleString('pt-BR')}</strong><small>{delta(deltaMetrics.opportunitiesCreated.absolute)}</small></button>
        <button type="button" onClick={() => openDrilldown('conversion')}><span>Conversão</span><strong>{percent(metrics.conversionRate)}</strong><small>{delta(deltaMetrics.conversionRate.absolute, 'percent')}</small></button>
        <button type="button" onClick={() => openDrilldown('sales')}><span>Ticket médio</span><strong>{money(metrics.averageTicketCents)}</strong><small>{delta(deltaMetrics.averageTicketCents.absolute, 'money')}</small></button>
        <button type="button" onClick={() => openDrilldown('cycle')}><span>Ciclo de venda</span><strong>{days(metrics.averageSalesCycleDays)}</strong><small>{delta(deltaMetrics.averageSalesCycleDays.absolute, 'days')}</small></button>
      </section>
      ...
      <section className="esmera-report-section esmera-report-evolution">
        ...
        <EChart option={chartOption} ariaLabel={evolutionMode === 'volume' ? 'Evolução diária de leads, oportunidades e vendas' : 'Evolução diária da receita'} height={340} />
      </section>
      ...
      <div className="esmera-report-two-column">
        <section className="esmera-report-section">
          <div className="esmera-report-section__header"><div><span className="esmera-eyebrow">Perdas</span><h2>Motivos registrados</h2><p>Ranking das razões estruturadas de perda.</p></div></div>
          {data.losses.length ? <div className="esmera-report-ranking">{data.losses.map((loss) => <button type="button" key={loss.reason} onClick={() => openDrilldown('loss-reason', loss.reason)}><div><span>{loss.label}</span><strong>{loss.volume}</strong></div><div className="esmera-report-ranking__track"><span style={{ width: `${Math.max(2, (loss.shareOfLosses || 0) * 100)}%` }} /></div><small>{percent(loss.shareOfLosses)}</small></button>)}</div> : <EmptyState title="Sem perdas no período" copy="Nenhuma oportunidade perdida corresponde aos filtros." />}
        </section>

        <section className="esmera-report-section">
          <div className="esmera-report-section__header"><div><span className="esmera-eyebrow">Origem</span><h2>Qualidade da aquisição</h2><p>Volume, conversão e receita atribuída por origem.</p></div></div>
          {data.sources.length ? <DataTable label="Performance por origem"><thead><tr><th>Origem</th><th>Volume</th><th>Conversão</th><th>Receita</th><th /></tr></thead><tbody>{data.sources.map((source) => <tr key={source.source}><td><button className="esmera-report-text-action" type="button" onClick={() => patchFilters({ source: source.source === 'unattributed' ? null : source.source })}>{sourceLabels[source.source] || source.source}</button></td><td>{source.opportunitiesCreated}</td><td>{percent(source.conversionRate)}</td><td>{money(source.revenueCents)}</td><td><button className="esmera-button esmera-button--quiet" type="button" onClick={() => openDrilldown('source', source.source)}>Registros</button></td></tr>)}</tbody></DataTable> : <EmptyState title="Sem origem mensurável" copy="Nenhuma oportunidade corresponde aos filtros." />}
        </section>
      </div>
      ...
      <section className="esmera-report-section">
        <div className="esmera-report-section__header"><div><span className="esmera-eyebrow">Equipe</span><h2>Performance por responsável</h2><p>Oportunidades, conversão, vendas e receita sem ranking decorativo.</p></div></div>
