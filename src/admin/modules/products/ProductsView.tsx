...
import { AccessDenied, ensureUser, findDocs, PageHeader, QueryError, TechnicalLink, ViewFrame } from '../../views/shared'
...
import type { ProductCategory, ProductDetail, ProductDocumentTab, ProductListItem, ProductsViewMode, ProductWorkspaceFilters } from './types'
...
function filtersFrom(params: Record<string, string | string[] | undefined>): ProductWorkspaceFilters {
  ...
  return { q: (first(params.q) || '').trim().slice(0, 100), status: first(params.status) || 'all', availability: first(params.availability) || 'all', publication: first(params.publication) || 'all', category: first(params.category) || 'all', page: integer(first(params.page), 1), limit: limitValue === 100 ? 100 : 50, view: viewValue === 'grid' ? 'grid' : 'list' as ProductsViewMode }
}
function whereFrom(filters: ProductWorkspaceFilters): Where | undefined {
  const and: Where[] = []
  if (filters.q) and.push({ or: [{ title: { like: filters.q } }, { subtitle: { like: filters.q } }, { code: { like: filters.q } }, { slug: { like: filters.q } }, { material: { like: filters.q } }] } as Where)
...
async function productDetail(props: AdminViewServerProps, id: string) {
  const product = await props.initPageResult.req.payload.findByID({ collection: 'products', id, depth: 2, draft: true, overrideAccess: false, user: props.initPageResult.req.user, req: props.initPageResult.req }) as unknown as ProductDetail
  const assessment = assessProductPublication(product)
  return { ...product, publicationReady: assessment.ready, publicationIssues: assessment.issues.map(({ message }) => ({ message })), revision: createDocumentRevision(product) }
}
export async function ProductsView(props: AdminViewServerProps) {
  ...
  try {
    ...
    const [result, categoryResult] = await Promise.all([
      findDocs<ProductListItem>(props.initPageResult.req, 'products', { sort: '-updatedAt', limit: filters.limit, page: filters.page, depth: 1, draft: true, where: whereFrom(filters), select: {
        id: true, title: true, subtitle: true, slug: true, code: true, catalogStatus: true, availability: true, _status: true,
        publicationReady: true, publicationIssues: true, publicationRevision: true, publicationContractVersion: true,
        publicationOperationalStatus: true, publicationVerificationStatus: true, publicationVerifiedAt: true, publicationTraceId: true,
        categories: true, gallery: true, priceMode: true, basePriceCents: true, updatedAt: true,
      } }),
      findDocs<ProductCategory>(props.initPageResult.req, 'categories', { sort: 'order', limit: 200, depth: 0, select: { id: true, title: true, slug: true } }),
    ])
