...
async function paramsOf(props: AdminViewServerProps) { return await Promise.resolve(props.searchParams as unknown as Record<string, string | string[] | undefined>) }
function filtersFrom(params: Record<string, string | string[] | undefined>): CategoryWorkspaceFilters { return { q: (first(params.q) || '').trim().slice(0, 100), status: first(params.status) === 'archive' ? 'archive' : 'active' } }
function whereFrom(filters: CategoryWorkspaceFilters): Where { const and: Where[] = [{ status: { equals: filters.status } } as Where]; if (filters.q) and.push({ or: [{ title: { like: filters.q } }, { slug: { like: filters.q } }, { 'searchTerms.term': { like: filters.q } }] } as Where); return { and } as Where }
function hierarchyDepth(category: CategoryParent, byId: Map<string, CategoryParent>) { const visited = new Set<string>([String(category.id)]); let depth = 0; let parentId = relationId(category.parent); while (parentId !== null && depth < 20) { const key = String(parentId); if (visited.has(key)) return depth; visited.add(key); const parent = byId.get(key); if (!parent) return depth + 1; depth += 1; parentId = relationId(parent.parent) } return depth }
async function selectedDetail(props: AdminViewServerProps, categoryId: string) { const category = await props.initPageResult.req.payload.findByID({ collection: 'categories', id: categoryId, depth: 2, draft: true, overrideAccess: false, user: props.initPageResult.req.user, req: props.initPageResult.req }) as unknown as CategoryDetail; return { ...category, revision: createDocumentRevision(category) } }
  ...
  try {
    const [visibleResult, allResult] = await Promise.all([
      findDocs<Omit<CategoryListItem, 'productCount' | 'depth'>>(req, 'categories', {
        ...
        select: {
          ...
          publicationOperationalStatus: true,
          publicationVerificationStatus: true,
          publicationVerifiedAt: true,
          publicationTraceId: true,
        },
      }),
    ...
    if (categoryId) {
      ...
      const [mediaResult, productsResult, productsCount] = await Promise.all([
        findDocs<CategoryMedia>(req, 'media', { sort: '-updatedAt', limit: 100, depth: 0, select: { id: true, filename: true, url: true, alt: true } }),
        findDocs<RelatedProduct>(req, 'products', { sort: 'title', limit: 50, depth: 0, draft: true, where: { categories: { contains: categoryId } } as Where, select: { id: true, title: true, code: true, catalogStatus: true, availability: true, _status: true } }),
    ...
    return (
      <ViewFrame props={props}>
        <PageHeader eyebrow="Catálogo" title="Categorias" subtitle="Taxonomia operacional em master-detail. Relações com produtos são derivadas; ordem e hierarquia permanecem governadas pelo Payload." actions={<TechnicalLink href="/admin/collections/categories/create" primary>Nova categoria</TechnicalLink>} />
        <div className={`esmera-categories-workspace${detail ? ' has-detail' : ''}`}>
          ...
          {detail ? <CategoryDetailView category={detail} tab={tab} filters={filters} categories={allResult.docs} media={media} termSuggestions={termSuggestions} relatedProducts={relatedProducts} relatedTotal={relatedTotal} /> : <section className="esmera-category-detail esmera-category-detail--empty"><span className="esmera-eyebrow">Detalhe</span><h2>Selecione uma categoria</h2><p>Abra uma linha para editar taxonomia, SEO, mídia e conferir os produtos relacionados sem sair do workspace.</p></section>}
        </div>
      </ViewFrame>
    )
  } catch (error) {
