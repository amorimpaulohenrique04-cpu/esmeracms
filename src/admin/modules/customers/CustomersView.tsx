...
export async function CustomersView(props: AdminViewServerProps) {
  ...
  try {
    ...
    if (filters.q) {
      ...
      if (matchingProducts.docs.length) {
        const byProduct = await findDocs<{ customer?: unknown }>(req, 'sales', {
          depth: 0, limit: 500, pagination: false,
    ...
    const [customerResult, usersResult, categoriesResult, productsResult] = await Promise.all([
      ...
      findDocs<ProductRef>(req, 'products', {
        sort: 'title', limit: 500, depth: 0, draft: true,
    ...
    const listSales = visibleIds.length ? await findDocs<SaleSummary>(req, 'sales', {
      sort: '-confirmedAt', limit: 1000, depth: 0,
    ...
    } as never) : { docs: [], totalDocs: 0 }

    const listItems: CustomerListItem[] = customerResult.docs.map((customer) => {
      const sales = (listSales.docs as Array<SaleSummary & { customer?: unknown }>).filter((sale) => String(relationId(sale.customer)) === String(customer.id) && purchaseStatuses.has(sale.status || ''))
      return {
        ...customer,
        ...
        {detail ? <section className="esmera-customer-detail">
          <header className="esmera-customer-detail__header">
            ...
            <div className="esmera-customer-detail__actions">
              ...
              {role === 'admin' ? <CustomerMergeDialog source={detail} customers={listItems} /> : null}
