 ...
 *
 * Single public bootstrap for the storefront shell + Home. It replaces four
 * uncached Payload REST calls made by the frontend on first render.
 */
export async function GET(request: Request) {
  const payload = await getPayload({ config })

  try {
    const result = await measureServerOperation('navigation', 'storefront.bootstrap.v1', async () => {
