...
type RelationDoc = { id: string | number; relatedTo?: unknown }
...
function customerData(input: Record<string, unknown> | undefined) {
  ...
  if (source.interestProfile && typeof source.interestProfile === 'object') {
    ...
    if (Array.isArray(profileSource.materials)) {
      profile.materials = profileSource.materials
        .map((item) => typeof item === 'string' ? item : item && typeof item === 'object' && 'value' in item ? String((item as { value?: unknown }).value || '') : '')
        .map((value) => value.trim())
        .filter(Boolean)
        .map((value) => ({ value }))
...
function replaceCustomerRelation(items: unknown, sourceId: string | number, targetId: string | number) {
  ...
  for (const item of items) {
    ...
    if (!next.some((candidate) => candidate.relationTo === relationTo && String(candidate.value) === String(replacement))) next.push({ relationTo, value: replacement })
  }
  return next
}

async function findRelationDocs(payload: Payload, user: unknown, collection: string, where?: Where) {
  return await payload.find({
    ...
    user: user as never,
    where,
  } as never) as unknown as { docs: RelationDoc[] }
}
...
async function transferDirectRelations(payload: Payload, user: unknown, sourceId: string | number, targetId: string | number) {
  ...
  for (const [collection, field] of relations) {
    const result = await findRelationDocs(payload, user, collection, { [field]: { equals: sourceId } } as Where)
    for (const doc of result.docs) {
      await payload.update({ collection: collection as never, id: doc.id, data: { [field]: targetId }, overrideAccess: false, user: user as never } as never)
    }
  }
}

async function transferPolymorphicRelations(payload: Payload, user: unknown, sourceId: string | number, targetId: string | number) {
  for (const collection of ['tasks', 'activities']) {
    const result = await findRelationDocs(payload, user, collection)
    for (const doc of result.docs) {
      const replaced = replaceCustomerRelation(doc.relatedTo, sourceId, targetId)
      if (JSON.stringify(replaced) === JSON.stringify(doc.relatedTo)) continue
      await payload.update({ collection: collection as never, id: doc.id, data: { relatedTo: replaced }, overrideAccess: true } as never)
    }
  }
}
    ...
    if (body.action === 'add-interest') {
      ...
      const interest = await payload.create({
        ...
        data: {
          ...
          source: 'manual',
          owner: user.id,
          notes: String(body.note || '').trim() || null,
      ...
      await payload.create({
        ...
        data: {
          ...
          kind: 'contact',
          occurredAt: new Date().toISOString(),
          summary: `Interesse adicionado: ${product.title}`,
          details: String(body.note || '').trim() || undefined,
          owner: user.id,
